...
The database consists of **6 tables** that manage an agency/organization's operations including:
...
Stores weekly progress reports submitted by users.
...
3. **JSON fields** (`progress`, `plans`, `blockers` in `WeeklyReport`) are stored as stringified JSON
4. **Nullable foreign keys** (`assigneeId`, `sprintId`, `ownerId`, `parentId`, `linkedKrId`) allow optional relationships

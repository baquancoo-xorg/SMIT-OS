  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="relative w-10 h-10 flex items-center justify-center rounded-full border border-border bg-surface text-text-2 shadow-sm transition-all hover:border-border-strong hover:text-accent-text hover:shadow-md"
      >
        <Bell size={20} className="text-on-surface-variant" />
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-4 h-4 px-1 bg-error text-on-error text-[10px] leading-none font-bold rounded-full flex items-center justify-center shadow-sm">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute right-0 top-full mt-2 w-80 bg-surface-2 rounded-card border border-border shadow-xl overflow-hidden z-50"
          >

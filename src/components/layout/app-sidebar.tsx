import { type FocusEventHandler, type MouseEventHandler, type PointerEventHandler } from 'react';
...
interface AppSidebarProps {
  ...
  onCollapsedNavClick?: MouseEventHandler<HTMLAnchorElement>;
...
function SidebarNavItem({
  ...
  return (
    <NavLink
      to={item.href}
      onClick={collapsed ? onCollapsedNavClick : onNavigate}
    ...
    >
      <div className="flex h-full flex-col">
        <div className={`flex h-[3.25rem] shrink-0 items-center border-b border-border ${collapsed ? 'justify-center' : 'gap-2.5 px-6'}`}>
          <SmitGridMark />
          {!collapsed && <span className="text-sm font-bold tracking-tight text-text-1">SMIT OS</span>}
        </div>
            ...
            <div key={section.label} className={collapsed ? 'space-y-3.5' : 'space-y-2'}>
              ...
              <div className={collapsed ? 'space-y-2.5' : 'space-y-1'}>
                {section.items.map(label => {
                  ...
                  return <SidebarNavItem key={item.href} item={item} collapsed={collapsed} active={active} onNavigate={onNavigate} onCollapsedNavClick={onCollapsedNavClick} />;
                })}
              </div>
        ...
        <div className={`relative shrink-0 border-t border-border ${collapsed ? 'px-0 py-4' : 'flex h-[4.5rem] items-center gap-1.5 px-5'}`}>
          {!collapsed && (
            <NavLink to="/profile" onClick={onNavigate} className="min-w-0 flex-1 rounded-[var(--radius-input)] py-1.5 text-left transition hover:text-text-1">
              <p className="truncate text-[10px] font-bold uppercase tracking-[0.04em] text-text-1">{currentUser?.fullName || 'Nguyễn Quân'}</p>

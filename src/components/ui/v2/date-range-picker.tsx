...
export interface DateRangePickerProps {
  ...
  label?: string;
  /** Custom preset list. Defaults to Today / Last 7d / Last 30d / This month / Last month / This quarter. */
...
export const DEFAULT_PRESETS: DateRangePreset[] = [
  { key: 'today', label: 'Today', range: () => ({ from: today(), to: endOfDay(today()) }) },
  {
    key: 'yesterday',
    label: 'Yesterday',
    range: () => {
  ...
  { key: '7d', label: 'Last 7 days', range: () => ({ from: subDays(today(), 6), to: endOfDay(today()) }) },
  { key: '30d', label: 'Last 30 days', range: () => ({ from: subDays(today(), 29), to: endOfDay(today()) }) },
  { key: 'tm', label: 'This month', range: () => ({ from: startOfMonth(today()), to: endOfDay(endOfMonth(today())) }) },
  {
    key: 'lm',
    label: 'Last month',
    range: () => {
...
export const DateRangePicker = forwardRef<HTMLDivElement, DateRangePickerProps>(
  ({ value, onChange, label = 'Date range', presets = DEFAULT_PRESETS, disabled, className = '' }, ref) => {
          ...
          {({ close }) => (
            <>
              <PopoverButton
                ...
                disabled={disabled}
                className={[
                  'inline-flex h-10 items-center gap-2 rounded-input border border-outline-variant bg-surface-container-lowest px-3',
                  'text-[length:var(--text-body)] text-on-surface',
              ...
              >
                <Calendar aria-hidden="true" className="size-4 text-on-surface-variant" />
              ...
              >
                ...
                <div className="flex w-1/2 flex-col gap-2 p-2">
                  ...
                  <button
                    ...
                    className="mt-1 inline-flex h-9 items-center justify-center rounded-button bg-primary text-[length:var(--text-body-sm)] font-semibold text-on-primary hover:bg-primary/90 focus-visible:outline-none"
                  >
                    Apply
                  </button>
                </div>

...
const STANDARD_CONTRACT: TableVariantContract = {
  ...
  headerCell: 'px-8 py-6 text-[length:var(--text-label)] font-semibold uppercase tracking-[var(--tracking-wide)] text-on-surface-variant',
  body: 'divide-y divide-outline-variant/30',
  row: 'group transition-colors hover:bg-primary/[0.02]',
  rowSelected: 'bg-primary/[0.04]',
  cell: 'px-8 py-5',
  actionHeaderCell: 'w-[104px] px-5 py-6 text-right text-[length:var(--text-label)] font-semibold uppercase tracking-[var(--tracking-wide)] text-on-surface-variant',
  actionCell: 'relative px-5 py-5 text-right',

...
import { MarketingMetrics, BlockerEntry, TodayPlanEntry, TaskEntry, BLOCKER_TAGS, AdHocTask } from '../../types/daily-report-metrics';
...
export default function MarketingDailyForm({ tasks, onClose, onSuccess }: MarketingDailyFormProps) {
  ...
  const [taskMetrics, setTaskMetrics] = useState<Record<string, MarketingMetrics>>({});
  ...
  const handleTaskStatusChange = (taskId: string, status: 'done' | 'doing') => {
    ...
    if (!taskMetrics[taskId]) {
      setTaskMetrics((prev) => ({
        ...prev,
        [taskId]: { campStatus: 'normal', channel: 'fb' },
      }));
    }
  };

  const updateTaskMetric = (taskId: string, field: keyof MarketingMetrics, value: string | number | boolean) => {
    setTaskMetrics((prev) => ({
      ...prev,
  ...
  const addBlocker = () => {
    setBlockers((prev) => [...prev, { id: `b${Date.now()}`, description: '', impact: 'none', tags: [] }]);
  };
  ...
  const appendBlockerTag = (id: string, tag: string) => {
    setBlockers((prev) => prev.map((b) => (b.id === id ? { ...b, description: b.description ? `${b.description} ${tag}` : tag } : b)));
  };

  const addTodayPlan = () => {
    setTodayPlans((prev) => [...prev, { id: `p${Date.now()}`, output: '', progress: 0, isPriority: false }]);
  };

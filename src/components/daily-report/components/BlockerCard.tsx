...
export default function BlockerCard({
  ...
  return (
    <div className="p-4 border border-red-100 bg-red-50/40 rounded-xl relative group">
      <button
        onClick={onRemove}
        className="absolute top-3 right-3 text-red-300 hover:text-red-500 transition-colors bg-white rounded-xl p-1 shadow-sm opacity-0 group-hover:opacity-100"
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-3 gap-4">
        <div className="md:col-span-2">
            ...
            <div className="flex gap-1.5 flex-wrap justify-end">
              {tags.map((tag) => (
                <button
                  ...
                  className="text-[10px] bg-white border border-slate-200 hover:bg-slate-100 px-1.5 py-0.5 rounded-md font-medium text-slate-600"
                >
                  {tag.replace(/\[|\]/g, '')}
                </button>
              ))}
          ...
          <textarea
            ...
            className="w-full text-sm p-3 rounded-xl border border-red-200 focus:border-red-400 focus:ring-1 focus:ring-red-400 outline-none min-h-[60px] bg-white placeholder-slate-400"

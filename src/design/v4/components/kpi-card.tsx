...
export function KpiCard({ label, value, delta, trend = 'up', icon, meta, className, padding = 'md', interactive = true, ...rest }: KpiCardProps) {
  return (
    <SurfaceCard padding={padding} interactive={interactive} className={cn('flex flex-col gap-xs', className)} {...rest}>
      ...
      <div className="flex items-baseline gap-sm">
        ...
        {delta && (
          <span className={cn('inline-flex items-center gap-xs rounded-pill px-sm py-xs text-caption font-semibold', trendClass[trend])}>

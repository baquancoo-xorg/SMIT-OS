...
export function CustomSelect<V extends string = string>({
  ...
  return (
    <div ref={containerRef} className={cn('relative', fullWidth ? 'w-full' : 'inline-block', className)}>
      {label && (
        <label htmlFor={reactId} className="block mb-tight text-label font-medium text-fg-muted">
          {label}
        </label>
      )}
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(
          'flex w-full items-center justify-between gap-snug h-11 px-cozy text-body',
      ...
      >
        <span className={cn('flex items-center gap-snug min-w-0', selected ? 'text-fg' : 'text-fg-faint')}>
          {selected?.icon && <span aria-hidden="true">{selected.icon}</span>}
          <span className="truncate">{selected?.label ?? placeholder}</span>
        </span>
        <ChevronDown size={16} aria-hidden="true" className="text-fg-subtle shrink-0" />
      </button>
      {open && (
        <div
          role="listbox"
          ...
          autoFocus
          className={cn(
            'absolute left-0 right-0 top-full mt-tight z-dropdown py-tight max-h-72 overflow-auto',
          ...
          {options.map((opt, i) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && <Check size={14} aria-hidden="true" className="text-accent" />}
              </button>
            );
          })}
        </div>
      )}
      {error ? <p className="mt-tight text-caption text-error">{error}</p> : helper ? <p className="mt-tight text-caption text-fg-subtle">{helper}</p> : null}
    </div>
  );
}

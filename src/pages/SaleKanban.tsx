...
export default function SaleKanban() {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <nav className="flex items-center gap-2 mb-2 text-on-surface-variant font-medium text-sm">
            ...
            <span className="text-on-surface">Kanban</span>
          </nav>
          <h2 className="text-4xl font-extrabold font-headline tracking-tight text-on-surface">Sales <span className="text-emerald-500">Kanban</span></h2>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-outline-variant/10 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-xl font-black font-headline">
              {items.reduce((sum, i) => sum + (i.dealValue || 0), 0) >= 1000000 
                ? `${(items.reduce((sum, i) => sum + (i.dealValue || 0), 0) / 1000000).toFixed(1)}M` 
                : items.reduce((sum, i) => sum + (i.dealValue || 0), 0).toLocaleString()}
            </h4>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-outline-variant/10 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-xl font-black font-headline">{items.filter(i => i.type === 'Deal').length}</h4>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-outline-variant/10 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-xl font-black font-headline">{items.length > 0 ? '32%' : '0%'}</h4>
          </div>
        </div>
      </div>
